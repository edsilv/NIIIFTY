...
import { fileConverter, remove, useAuthoringFile } from "@/hooks/useFile";
...
export function EditFile({ id }: {
  ...
  const [license, setLicense] = useState<LicenseURL>("https://creativecommons.org/publicdomain/zero/1.0/");
    ...
    return (
      <>
        ...
        >
          ...
          {/* title */}
          <label htmlFor="title" className="mt-8 font-light text-gray-600">
          ...
          <input
            ...
            name="title"
            maxLength={200}
            className="border-b border-gray-300 bg-transparent py-2 pl-4 font-light text-gray-600 ring-blue-500 focus:rounded-md focus:outline-none focus:ring-1"
          />
          {formErrors?.title && (
            <ErrorMessage><>{t("titleInvalid")}</></ErrorMessage>
          )}
          {/* attribution */}
          <label htmlFor="attribution" className="mt-8 font-light text-gray-600">
            <>{t("attribution")}</>
          </label>
          <input
            type="text"
          ...
          />
          {/* license */}
          <label htmlFor="license" className="mt-8 font-light text-gray-600">
            <>{t("license")}</>
          </label>
          <select
            value={license}
          ...
          {/* cid */}
          <label htmlFor="cid" className="mt-8 font-light text-gray-600">
            <>{t("cid")}</>
          </label>
          ...
          <div className="flex flex-row items-center justify-start">
            <button
              type="submit"
              className={cx(
                "transition-color mt-8 flex flex-row items-center rounded-md bg-black px-6 py-3 text-lg font-medium text-white shadow-md duration-500 hover:bg-white hover:text-black focus:outline-none",
              )}
            >
              <>{t("update")}</>
            </button>
            <button
              ...
              className={cx(
                "transition-color mt-8 ml-4 flex flex-row items-center rounded-md bg-gray-300 px-6 py-3 text-lg font-medium text-black shadow-md duration-500 hover:bg-white hover:text-black focus:outline-none",
              )}
            >
              <>{t("delete")}</>
            </button>
